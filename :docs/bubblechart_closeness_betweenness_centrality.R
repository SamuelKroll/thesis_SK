...
library(MASS)
setwd("/Volumes/biodata/dep_psl/grp_kemen/working_directory_sam/Sequencing/Time_course/New_analysis/Networks/Hub_formation/")

#rohdaten einlesen
data<-read.csv("march.txt", sep="\t", dec = ".")
str(data)
...
fitlognorm <- fitdistr(log(data$BetweennessCentrality[which(data$BetweennessCentrality>0)]), "normal")
...
bclen <- length(which(data$BetweennessCentrality > bcmin))
# List of nodes above this level
list <- which(data$BetweennessCentrality > bcmin)
bclist <- data$Taxonomy[list]

# Same for Closeness Centrality 

# Fit log-transformed data to normal distribution
fitlognorm_close <- fitdistr(log(data$ClosenessCentrality[which(data$ClosenessCentrality>0)]), "normal")
...
# Number of nodes above this level
bclen_close <- length(which(data$ClosenessCentrality > bcmin_close))
# List of nodes above this level
list_close <- which(data$ClosenessCentrality > bcmin_close)
...
expression <- ggplot(data, aes(x = ClosenessCentrality, y= BetweennessCentrality, fill= Organism, size =Degree, ymax=max(BetweennessCentrality)*1.1)) +
...
ggsave("all_years_combined.pdf", useDingbats = FALSE)
